function [ data, g, data0 ] = TwoPlayerGame(accuracy)
...
tMax = 15.8;                  % End time.
plotSteps = 19;               % How many intermediate plots to produce?
t0 = 0;                      % Start time.
...
schemeData.hamFunc = @TwoPlayerGameHamFunc;
schemeData.partialFunc = @TwoPlayerGamePartialFunc;
schemeData.grid = g;
...
while(tMax - tNow > small * tMax)
  ...
  figure(f1);
  [ view_mesh_az, view_mesh_el ] = view;
  ...
  view(view_mesh_az, view_mesh_el);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%---------------------------------------------------------------------------
function hamValue = TwoPlayerGameHamFunc(t, data, deriv, schemeData)
...
% hamValue = -(schemeData.velocityA * deriv{1} ...
% 	       +deriv{2} * schemeData.inputB .* abs(grid.xs{1} + 1 - ... 
%            schemeData.velocityA / ...
%            schemeData.inputB)./ (grid.xs{1} + 1));

 hamValue = -(abs(schemeData.velocityA * sqrt(deriv{1}.^2+(deriv{2}.^2 ...
             ./(1+grid.xs{1}).^2))) - (sign(deriv{2})));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%---------------------------------------------------------------------------
function alpha = TwoPlayerGamePartialFunc(t, data, derivMin, derivMax, schemeData, dim)
...
switch dim
  ...
  case 2
     alpha = schemeData.velocityA./(grid.xs{1}+1)+1;
%     alpha = schemeData.velocityA;
